...
export const accessControlAbi = [
  "function triggerEmergencyAccess(address patient, bytes32 reasonHash, uint256 duration)",
  "function hasEmergencyAccess(address patient, address grantee) view returns (bool)",
] as const;
...
export async function triggerEmergencyAccess(
...
): Promise<{ txHash: string; expiresAt: Date }> {
  const { signer } = await connectWallet();
  const contractAddress = process.env.NEXT_PUBLIC_CONTRACT_ADDRESS;
  if (!contractAddress) throw new Error("NEXT_PUBLIC_CONTRACT_ADDRESS chua duoc cau hinh.");
  ...
  const tx = await (contract as any).triggerEmergencyAccess(
    patientWallet,
    reasonHash,
  ...
  return { txHash: receipt.hash, expiresAt };
}
