...
class AccessControlFlowTests {
    ...
    @Test
    void doctorVerifiesEmergencyAccessAndWrongRoleIsDenied() throws Exception {
        Setup setup = setup();

        mvc.perform(post("/access-control/emergency/verify")
                        .header("Authorization", bearer(setup.doctorToken()))
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(emergencyRequest(setup)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.patientProfileId").value(setup.patientId()))
                .andExpect(jsonPath("$.data.doctorProfileId").value(setup.doctorId()))
                .andExpect(jsonPath("$.data.active").value(true));

        mvc.perform(get("/access-control/emergency/doctors/me")
                        .header("Authorization", bearer(setup.doctorToken())))
                .andExpect(status().isOk())
    ...
    @Test
    void doctorAccessRequestIncludesWalletForPatientApproval() throws Exception {
        Setup setup = setup();

        mvc.perform(post("/access-requests")
                        .header("Authorization", bearer(setup.doctorToken()))
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"patientProfileId":%d,"reason":"Need to review recent labs"}
                                """.formatted(setup.patientId())))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.doctorProfileId").value(setup.doctorId()))
                .andExpect(jsonPath("$.data.doctorWallet").value(setup.doctorWallet()))
                .andExpect(jsonPath("$.data.status").value("PENDING"));

        mvc.perform(get("/access-requests/patients/me")
                        .header("Authorization", bearer(setup.patientToken())))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.content[0].doctorWallet").value(setup.doctorWallet()));
    }
    ...
    private String emergencyRequest(Setup setup) {
        return """
                {
                  "patientProfileId":%d,
                  "patientWallet":"%s",
                ...
                """.formatted(setup.patientId(), setup.patientWallet(), setup.doctorWallet(), hash('e'));
    }
