...
public class DoctorProfileService {
    ...
    public DoctorProfileResponse upsertMyProfile(Long userId, DoctorProfileRequest request) {
        ...
        InstitutionProfile institution = request.institutionId() == null
                ? null
                : institutionProfileRepository.findById(request.institutionId())
        ...
        // Link to institution with PENDING status (institution must approve)
        if (institution != null) {
            boolean alreadyLinked = institution.equals(profile.getInstitutionProfile());
            if (!alreadyLinked) {
                profile.setInstitution(institution, "PENDING");
            }
        }
    ...
    private DoctorProfileResponse toResponse(DoctorProfile profile) {
        ...
        var institution = profile.getInstitutionProfile();
        return new DoctorProfileResponse(
                ...
                institution == null ? null : institution.getInstitutionName(),
                profile.getInstitutionStatus(),
                profile.getPhone(),
