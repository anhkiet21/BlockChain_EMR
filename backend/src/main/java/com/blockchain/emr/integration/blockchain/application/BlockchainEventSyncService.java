...
public class BlockchainEventSyncService {
    ...
    public SyncResult sync() {
        ...
        if (from.compareTo(latest) > 0) {
            return new SyncResult(from, latest, 0);
        }
        ...
        return new SyncResult(from, latest, recordCount);
    }

    public record SyncResult(BigInteger fromBlock, BigInteger toBlock, int recordEvents) {}
}
